...
'TATA, Sundravadanam (365-External)': ['S365BID1P052.in365.corpintra.net', 's365ild1p019.in365.lia.corpintra.net', 't365bid1infr123.in365.corpintra.net'],
...
def filter_scope_and_save_excel(input_file):
    ...
    df = pd.read_excel(input_file, sheet_name="Apr'24", header=0)

    print("Striping any leading/trailing whitespace characters from the column names")
    df.columns = df.columns.str.strip()

    print("Verifying if 'Scope Infosys/Daimler' exists in the columns")
    if 'Scope Infosys/Daimler' not in df.columns:
        print("Column 'Scope Infosys/Daimler' not found in the input file.")
        print(f"Available columns are: {df.columns.tolist()}")
        return

    print("Filtering the rows where 'Scope Daimler/Infosys' contains 'Daimler'")
    filtered_df = df[df['Scope Infosys/Daimler'].str.contains('Daimler', na=False)]

    return filtered_df

def filter_hostname_and_save(input_file, email_template):
    filtered_df = filter_scope_and_save_excel(input_file)
    if filtered_df is None:
        return

    if 'HostName' not in filtered_df.columns:
        print("Column 'HostName' not found in the filtered data.")
        print(f"Available columns are: {filtered_df.columns.tolist()}")
        return

    current_date = datetime.datetime.now()
    year_suffix = current_date.strftime("%y")
    base_name = os.path.basename(input_file)
    name_part = "linux" if "linux" in base_name.lower() else "windows" if "windows" in base_name.lower() else "unknown"

    owner_dfs = {owner: pd.DataFrame(columns=filtered_df.columns) for owner in server_owners.keys()}

    for owner, hostnames in server_owners.items():
        for hostname in hostnames:
            filtered_data = filtered_df[filtered_df['HostName'].str.contains(hostname, na=False)]
            if not filtered_data.empty:
                owner_dfs[owner] = pd.concat([owner_dfs[owner], filtered_data])

    for owner, df in owner_dfs.items():
        if not df.empty:
            file_name = f"{owner}_Daimler_{name_part}_VMR_Apr_{year_suffix}.xlsx"
...
def send_email_with_attachment(recipient_email, attachment_path, owner, email_template):
    ...
    full_attachment_path = rf"C:\Users\KPAKKIA\Documents\Soc_Automation_Scripts\{attachment_path}"


    # Replace placeholders in the subject and body
    # current_month = datetime.datetime.now().strftime("%B")
    current_month = "October"
    subject = f"Vulnerability Report and Required Remediation Actions for {current_month} 2024"
    email_body = re.sub(r'{MONTH}', current_month, email_body)
    email_body = re.sub(r'{NAME}', owner, email_body)
    print(f"Attachment Path: {full_attachment_path}")

    # Example of including an image in the email body
    # image_path = rf'C:\Users\KPAKKIA\Documents\Soc_Automation_Scripts\VM_image.jpg'  # Replace with the path to your image file
    # image_cid = 'image1'  # Unique content ID for the image


    # Create new mail item and set email fields
    mail = outlook.CreateItem(0)
    mail.Display()
    mail.To = recipient_email
    mail.cc = "dw_365-dicv_itsecurity@daimlertruck.com"
    # mail.cc = ""
    mail.Subject = subject

    # Convert the image to base64 and embed it in the HTML body
    # image_path = 'VM_image.jpg'  # Replace with the path to your image file
    # with open(image_path, 'rb') as img_file:
    #     img_data = img_file.read()
    #     img_data_base64 = base64.b64encode(img_data).decode('utf-8')
    #     img_tag = f'<img src="data:image/jpeg;base64,{img_data_base64}" alt="VM Image">'

    # # Replace the placeholder in the HTML body with the image tag
    # email_body = email_body.replace('{IMAGE}', img_tag)

    mail.HTMLBody = email_body
    mail.Attachments.Add(full_attachment_path)
    mail.Importance = 2  # Mark the email as high importance

    # Set the sender's email address if required
